    ...
    function myLibrary(){
                    ...
                    setTimeout(function(){
                        while(i<catalog.length){
                            if(Math.abs(catalog[i].price-price)<=difference){
                                priceArray.push({id: i, price: catalog[i].price, type:catalog[i].type});
                            }
                            i++;
                        }
                        resolve(priceArray);
        ...
        function searchProductsByType(type){
            var promise=new Promise(function(resolve,reject){
                var i=0;
                var typeArray=[];
                var possibleTypes=['Electronics', 'Clothing', 'Book', 'Food'];
                    ...
                    setTimeout(function() {
                        while(i<catalog.length){
                            if(catalog[i].type==type){
                                typeArray.push({id: i, price: catalog[i].price, type: catalog[i].type})
                            }
                            i++;
                        }
                        resolve(typeArray);
        ...
        function createRandomProduct(){
            var typeArray=['Electronics','Clothing', 'Book', 'Food'];
